	...
	Tree.prototype = {
		...
		_collapseAllButThis: function(node) {
		    ...
		    while(cNode) {
		        ...
		        cNode = this.nodes[cNode.parentId];
		    }

		    for(i = 0; i < this.nodes.length; ++i) {
		        if(ids.indexOf(i) == -1 && this.nodes[i].nodes) {
		            this.nodes[i]._nodes = this.nodes[i].nodes;
		            delete this.nodes[i].nodes;
		        }
		...
		_render: function() {
			...
			// Build tree
			self.nodes = [];
			self._buildTree(self.tree, 0, -1);
		},

		// Starting from the root node, and recursing down the
		// structure we build the tree one node at a time
		_buildTree: function(nodes, level, parentId) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				if (node.nodes) {
					return self._buildTree(node.nodes, level, node.nodeId);
				}
			});
		},
